...
| `user_plates/` | Kullanıcı tarafından internetten toplandı (bkz. `docs/decisions.md` #46) | Plaka **tespiti** — 1.955 gerçek Türk plakası fotoğrafı, yüksek çözünürlük (ör. 4608x2592), tek sınıf, zaten temiz bbox formatında. Görsel olarak doğrulandı: gerçek trafik/park sahneleri, okunaklı plakalar (ileride OCR etiketleme için de kaynak olabilir). | **Netleşti (2026-08-18):** kullanıcı bunların internetten topladığı görüntüler olduğunu ve bu görüntülerle eğitilen checkpoint'in (`models/plate_detector/best.pt`) yayınlanabileceğini onayladı — bkz. `docs/decisions.md` #46. Ham görüntülerin kendisi hâlâ git'e girmiyor, sadece onlarla eğitilmiş checkpoint yayınlanıyor. |
| `test_videos/`, `test_foto/` | **Kullanıcının kendi çektiği** kişisel test video/fotoğrafları (`arac2.mp4`, `arac3.mp4`, `foto.jpeg`) | Pipeline'ı gerçek görüntüyle doğrulamak için (bkz. `docs/decisions.md` #28 ve sonrası) — geliştirme sürecinde yoğunca kullanıldı. | **Asla yayınlanmayacak** (kullanıcının açık talebi, `docs/decisions.md` #46) — zaten `data/external/*` gitignore kapsamında, hiçbir release/commit'e dahil edilmiyor; bu satır sadece niyeti açıkça kayıt altına almak için. |
| `speed_eval/` | Kullanıcı tarafından çekilir (yerel, kişisel test klipleri) | Hız tahmini doğruluk ölçümü için — ölçülü mesafe + kronometre ile çekilen klipler + `manifest.csv` (`scripts/eval_speed_estimation.py`, bkz. `docs/decisions.md` #43). Klasör/format zorunlu değil, sadece `scripts/eval_speed_estimation.py`'nin dokümante ettiği manifest kolonlarına uyması yeterli. | Kişisel/yerel, test_videos ile aynı kategoride — **asla yayınlanmayacak.** |
